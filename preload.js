...
contextBridge.exposeInMainWorld("electronAPI", {
    ...
    calculateSatellitePositions: (satellites, timestamp) =>
        ipcRenderer.invoke("calculate-satellite-positions", satellites, timestamp),
     ...
     */
    calculatePositionsInterpolated: (satellites, timestamp, deltaMs = 100) =>
        ipcRenderer.invoke("calculate-positions-interpolated", satellites, timestamp, deltaMs),
     ...
     */
    generateOrbitPath: (tle, startTimestamp, numPoints = 100) =>
        ipcRenderer.invoke("generate-orbit-path", tle, startTimestamp, numPoints),
     ...
     */
    generateOrbitPathsBatch: (satellites, startTimestamp, numPoints = 100) =>
        ipcRenderer.invoke("generate-orbit-paths-batch", satellites, startTimestamp, numPoints),
     ...
     */
    generateCoverageCircle: (center, radiusKm, nPoints = 72) =>
        ipcRenderer.invoke("generate-coverage-circle", center, radiusKm, nPoints),
     ...
     */
    calculateCoverageRadius: (altitudeKm) =>
        ipcRenderer.invoke("calculate-coverage-radius", altitudeKm),
