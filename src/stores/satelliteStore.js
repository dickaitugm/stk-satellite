      ...
      (set, get) => ({
        ...
        addSatellite: (satellite) => {
          ...
          if (!licenseStore.canAddObject("satellite", get().satellites.length)) {
            ...
            return {
              success: false,
              error: "LIMIT_REACHED",
              message: `Free tier is limited to ${licenseStore.tierLimits.maxSatellites} satellite(s). Please activate a license to add more.`,
            };
          }
        ...
        removeSatellite: (id) =>
          set((state) => ({
            ...
            selectedSatelliteId: state.selectedSatelliteId === id ? state.satellites[0]?.id : state.selectedSatelliteId,
          })),
        ...
        toggleVisibility: (id) =>
          set((state) => ({
            satellites: state.satellites.map((s) => (s.id === id ? { ...s, isVisible: !s.isVisible } : s)),
          })),
