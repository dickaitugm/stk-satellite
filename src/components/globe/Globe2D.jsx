...
const Globe2D = ({ onMouseMove }) => {
            ...
            if (isStableRef.current) {
                ...
                if (maxLat > minLat) {
                    if (currentLat > maxLat)
                        wwdRef.current.navigator.lookAtLocation.latitude = maxLat;
                    if (currentLat < minLat)
                        wwdRef.current.navigator.lookAtLocation.latitude = minLat;
                }
            }
        };
    ...
    useEffect(() => {
        ...
        console.log(
            `🚀 Initializing WorldWind with dimensions: ${dimensions.width} x ${dimensions.height}`
        );
        ...
        const initTimeout = setTimeout(() => {
            try {
                ...
                console.log(
                    `📏 Canvas client size: ${canvas.clientWidth} x ${canvas.clientHeight}`
                );
